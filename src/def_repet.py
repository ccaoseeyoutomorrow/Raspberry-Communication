    ...
    def __init__(self,Node_list_R,Node_list_T):
        ...
        for i in range(len(self.Speed_list)):
            for j in range(len(self.Speed_list)):
                temp=min(abs(i+8-j),abs(i-j))
                if(temp!=0):
                    biospi=(4-temp)*22.5/360*math.pi
                    bios=-0.2062*biospi*biospi-0.0096*biospi+1.0067
                    temppi=1-0.2*((4-temp)*45/360*math.pi)*((4-temp)*45/360*math.pi)
                    self.Speed_list[i][j]=self.Speed_list[i][j]/temppi
...
def is_inner(vertxy,testy,testx):
    j= len(vertxy)-1
    flag=False
    for i in range(len(vertxy)-1):
...
def def_show():
    Node_location_A = [(0,-12.4),(6.72,-6.72),(8,0),(5.87,5.87),(0,9.9),(-8.63,8.63),(-13.4,0),(-9.26,-9.26)]
    Node_location_B = [(0,-12.4),(6.72,-6.72),(8,0),(5.87,5.87),(0,9.9),(-8.63,8.63),(-13.4,0),(-9.26,-9.26)]
...
if __name__ == '__main__':
    ...
    # Node_location_A = [(0,-12.4),(6.72,-6.72),(8,0),(5.87,5.87),(0,9.9),(-8.63,8.63),(-13.4,0),(-9.26,-9.26)]
    # Node_location_B = [(0,-12.4),(6.72,-6.72),(8,0),(5.87,5.87),(0,9.9),(-8.63,8.63),(-13.4,0),(-9.26,-9.26)]
    # Node_Number_A = len(Node_location_A)  # 节点数量
    # Node_Number_B = len(Node_location_B)
    # Node_list_A = Node_update(Node_location_A)
    # Node_list_B = Node_update(Node_location_B)
    # Ultra_Line = Ultrasonic_Line(Node_list_A, Node_list_B)
    # cell_100 = Cell()
    # cell_100.updata_V(Ultra_Line, Node_list_A, Node_list_B)
    # yuzhi=np.median(cell_100.V)
    # ultra_ray(Ultra_Line.Speed_list, Node_list_A, Node_list_B,yuzhi)
    # ultra_ellipse(Ultra_Line.Speed_list, Node_list_A, Node_list_B, Ultra_Line.Distance_list, Ultra_Line.B,yuzhi)
    # show_plt(cell_100.V,yuzhi)
