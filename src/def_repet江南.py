...
class Ultrasonic_Line():
    def __init__(self,Node_list_R,Node_list_T,file_time_name,agflag):
        ...
        # 根据无损区域的速度，来更新时间误差
        if agflag==1:
            self.江南_updateV(file_time_name)
    ...
    def 江南_updateV(self,file_time_name):
        # 时间list赋值
        data_list=readfile(file_time_name)
        # 计算时间补偿
        ...
        for i in range(count_num):
            count_array[i]=[i,int(sorted_speed[-i-1]/8),sorted_speed[-i-1]%8]
        count_array1=[[1,0,1],[2,0,2],[2,1,2],[3,2,6],[4,3,6],[5,4,6],[6,6,7]]
        count_array1=[[0,0,1]]
        ...
        i=count_array[0][1] #默认接收传感器位置
        j=count_array[0][2] #默认发送传感器位置l
        dij=self.Distance_list[i][j]
        bij=self.bias[i][j]
        tij=self.Time_list[i][j]
        for c,n,m in count_array:
            # ti=t[de0] 要求的系数
            # tj=t[de1] 要求的系数
            dnm=self.Distance_list[n][m]
            bnm=self.bias[n][m]
            tnm=self.Time_list[n][m]
            # tn=t[n] 要求的系数
            # tm=t[m] 要求的系数
...
class Cell():
    def __init__(self,radius):
        ...
        cell_length = (radius+1)*2/Cell_Number
        ...
        for i in range(Cell_Number):
            for j in range(Cell_Number):
                if (self.X[i][j]*self.X[i][j]+self.Y[i][j]*self.Y[i][j])<=radius*radius:
                    self.inner[i][j]=True
        # 判断点是否在多边形内
        # for i in range(Cell_Number):
        #     for j in range(Cell_Number):
    ...
    def update_RV(self,Line_list):
        ...
        # for i in range(Cell_Number):
        #     for j in range(Cell_Number):
        #         #如果点在园内且未受到影响，则它的值为四周的平均值
        #         if self.inner[i][j] and self.V[i][j]==2:
        #             self.V[i][j]=(np.sum(self.V[i-4:i+5,j-4:j+5])-1)/63
...
def show_heatmap(list_v,agflag):
    ...
    blue_red1 = LinearSegmentedColormap(cmap_name, cdict1)
    ...
    # plt.savefig('testblueline.png',bbox_inches= 'tight',dpi=400)
    plt.show()
...
def heat_ray(Speed_list,Node_list_A,Node_list_B,yuzhi):
    fig, ax=plt.subplots()  # 更新
    ax
    plt.xlim(-15, 15)
    plt.ylim(-15,15)
    for i in range(NodeA_num):
        for j in range(NodeB_num):
            ax.plot([Node_list_A[i].x, Node_list_B[j].x],[Node_list_A[i].y, Node_list_B[j].y],)

    fig.suptitle(time.strftime("%m%d%H%M%S", time.localtime())+'ultra_ray')
    plt.show()

def show_smallellipse(small_ellipse,Node_locat):
    fig, ax=plt.subplots()  # 更新
    fig.suptitle('small_ellipse')
    plt.xlim(-17, 17)
    plt.ylim(-17,17)
    for i in range(NodeA_num):
        for j in range(NodeB_num):
            if i!=j:
                for n in range(small_ellipse[i][j].num):
                    color='red'
                    长轴=small_ellipse[i][j].L*2
                    短轴=small_ellipse[i][j].B*2
                    x1=small_ellipse[i][j].XYaxis[n][0][0]
                    y1=small_ellipse[i][j].XYaxis[n][0][1]
                    x2=small_ellipse[i][j].XYaxis[n][1][0]
                    y2=small_ellipse[i][j].XYaxis[n][1][1]
                    dx=x1-x2
                    dy=y1-y2
                    angle = np.arctan2(dy, dx)
                    angle = np.degrees(angle)
                    e = Ellipse(xy=small_ellipse[i][j].cirXY[n],width=长轴,
                                height=短轴, angle=angle,facecolor=color, alpha=0.1)
                    ax.add_artist(e)
                plt.plot([Node_locat[i].x, Node_locat[j].x],[Node_locat[i].y, Node_locat[j].y], color=color)
    plt.show()
...
def ultra_ellipse(Speed_list,Node_list_A,Node_list_B,长轴,短轴,yuzhi):
    ...
    :param Node_list_A: 节点坐标
    :param Node_list_B: 节点坐标
    :param 长轴: 长轴距离
    :param 短轴: 短轴长度
    :param yuzhi: 阈值
    """
...
def def_show():
    agflag=0
    file_time_name=r'D://02_soft_temp/py_temp/树莓派数据通信/江南data/202012241136.txt'
    radius=15 #检测树木传感器的位置半径
    Node_location_A=[]
    Node_location_B=[]
    for i in range(NodeA_num):
    ...
    # show_smallellipse(small_ellipse,Node_list_A) #显示射线上的小椭圆
    cell_100 = Cell(radius)
    cell_100.update_RV(small_ellipse)
    cell_100.updata_UV(Ultra_Line.Speed_list, Node_list_A, Node_list_B,4,4)
    ...
    # write_excel(cell_100.V,'test.xlsx')
