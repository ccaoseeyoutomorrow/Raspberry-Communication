...
class Ultrasonic_Line():
    def __init__(self,Node_list_R,Node_list_T,file_time_name):
        ...
        #假设T补偿所有传感器位置一致
        # Vtrue=V'*(T'-T补)/T'
        # 则有 T补=(Vi-Vj)*Ti*Tj/(Vi*Tj-Vj*Ti)
        for i in range(1,7):
            temp1=(self.Speed_list[0][i]-self.Speed_list[0][i+1])*self.Time_list[0][i]*self.Time_list[0][i+1]
            temp2=self.Speed_list[0][i]*self.Time_list[0][i+1]-self.Speed_list[0][i+1]*self.Time_list[0][i]
            self.timebias =temp1/temp2
            print(self.timebias)
        # 对原始时间加上偏置，重新计算
        count=0
        for i in range(NodeA_num):
            for j in range(i+1,NodeB_num):
                if i!=j:
                    # 给时间赋值，单位：毫秒
                    self.Time_list[i][j]=data_list[count]+self.timebias
                    self.Time_list[j][i] = data_list[count]+self.timebias
                else:
                    self.Time_list[j][i]=0
                count+=1
        self.Speed_list =np.divide(self.Distance_list,self.Time_list) # 速度list赋值
...
# 找出nplist中比thres大的，第num个小的数值，xnum、ynum为
