...
def def_show(file_time_name):
    ...
    yuzhi=find_yuzhi(Ultra_Line.Speed_list,-10,NodeA_num,NodeB_num)
    small_ellipse=[[]for i in range(NodeA_num)]
    for i in range(NodeA_num):
        for j in range(NodeB_num):
            # pass
            small_ellipse[i].append(RSEN(Ultra_Line,i,j,line_minst,Node_list_A))
    # ultra_ellipse(small_ellipse[0][3].Speed_list, Node_list_A, Node_list_B, 0+.Distance_list, Ultra_Line.B,yuzhi) #绘制小椭圆
    cell_100 = Cell(10,12)
    ...
    cell_100.sperate_jianbian()
    label=cell_100.re_label(0.3)
    np.savetxt("test1_szh.txt", label, fmt='%d', delimiter=' ')
    # show_heatmap(cell_100.V,agflag)


if __name__ == '__main__':
    file_time_name= r'Data/实验室1号树木/手按测量.txt'
    # def_show(file_time_name)
    # read_show('label1.txt')
